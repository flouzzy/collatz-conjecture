...
### Lemma 52: Strong mixing property of the adelic dynamics

**1. Statement of Lemma 52**
The measured dynamical system $(\mathcal{G}_{\mathbb{A}}, \mathcal{B}_{\mathbb{A}}, \nu_{\mathbb{A}}, \mathcal{T}_{\mathbb{A}})$ is strongly mixing. That is, for all measurable sets $A, B \in \mathcal{B}_{\mathbb{A}}$, we have:
$$\lim_{n \to \infty} \nu_{\mathbb{A}}(A \cap \mathcal{T}_{\mathbb{A}}^{-n}(B)) = \nu_{\mathbb{A}}(A) \nu_{\mathbb{A}}(B)$$

**2. Proof of Lemma 52**

**Step 1: Connection with the eigenfunctions of the transfer operator**
Let $A$ and $B$ be two measurable sets of the adelic fibration $\mathcal{G}_{\mathbb{A}}$. The normalized transfer operator $\mathcal{L}$ is defined such that its iterate $\mathcal{L}^n$ acts on the observables.
By duality with respect to the conformal measure $\mu_0$, we have the identity:
...
Let us switch to the invariant probability measure $\nu_{\mathbb{A}} = h \cdot \mu_0$.
The mixing expression becomes:
...
**Step 2: Spectral convergence of the transfer operator**
The Ruelle-Perron-Frobenius theorem applied to our operator $\mathcal{L}$ with a Hölder potential guarantees that $\mathcal{L}$ admits a spectral gap on the space of Hölder continuous functions. The operator can be written in the form:
$$\mathcal{L} = \lambda_0 \Pi + \mathcal{R}$$
where $\Pi$ is the projection onto the principal eigenspace (generated by $h$, corresponding to the maximal eigenvalue $\lambda_0 = 1$ after normalization of the measure), and the remainder $\mathcal{R}$ satisfies $\mathcal{R}\Pi = \Pi\mathcal{R} = 0$, with a spectral radius strictly less than 1: $\rho(\mathcal{R}) < 1$.

**Step 3: Application to asymptotic iterates**
For any observable function $\phi$, the $n$-th power of the transfer operator acts asymptotically as:
$$\mathcal{L}^n(\phi) = \Pi(\phi) + \mathcal{R}^n(\phi)$$
The projection $\Pi$ projects onto the direction of $h$ by integrating against $\mu_0$: $\Pi(\phi) = \left( \int \phi \, d\mu_0 \right) h$.
Let us apply this to the function $\phi = 1_A \cdot h$. Since the invariant probability measure is $\nu_{\mathbb{A}} = h \mu_0$, the integral of $\phi$ with respect to $\mu_0$ is $\nu_{\mathbb{A}}(A)$.
Thus, we obtain:
$$\lim_{n \to \infty} \mathcal{L}^n(1_A \cdot h) = \left( \int 1_A \cdot h \, d\mu_0 \right) h = \nu_{\mathbb{A}}(A) \cdot h$$
The convergence holds in the uniform norm.

**Step 4: Conclusion of strong mixing**
Substituting this limit into the integral of mixing, we obtain:
...
Which proves the strong mixing property.

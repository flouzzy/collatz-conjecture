...
**Lemma 52 (Absence of Non-Ergodic Diffuse Invariant Radon Measures):**
...
### Lemma 47: Cohomological Invariance of Fibered Cycles
**Statement:** Let $\mathcal{H}^1(\mathcal{G}_{\mathbb{A}}, \mathbb{Z})$ be the first cohomology group of the adelic fibration. Every finite dynamical cycle corresponds to a trivial class in this group, implying the uniqueness of the trivial cycle modulo cohomological equivalence.

**Step 1: Definition of the cohomological boundary operator**
We define the chain complex associated with the dynamics on $\mathcal{G}_{\mathbb{A}}$. Let $C_0$ be the free abelian group generated by the Syracuse trajectories and $C_1$ be the group generated by the transitions $\mathcal{T}_{\mathbb{A}}$.
The boundary operator $\partial : C_1 \to C_0$ is defined by $\partial(x \to \mathcal{T}_{\mathbb{A}}(x)) = \mathcal{T}_{\mathbb{A}}(x) - x$.
A trajectory forms a finite cycle if and only if the integration of the transitions over this cycle has a null boundary.

**Step 2: Construction of the modular intersection form**
Let $\omega$ be the adelic differential 1-form canonically associated with the valuation flow, defined by $\omega = d(\log_{\mathbb{A}}(x))$ where $\log_{\mathbb{A}}$ is the adelic logarithm.
The integration of $\omega$ along a closed cycle $\gamma$ of length $L$ is written $\oint_\gamma \omega = \sum_{k=0}^{L-1} \left(\log_{\mathbb{A}}(\mathcal{T}_{\mathbb{A}}^{k+1}(z_0)) - \log_{\mathbb{A}}(\mathcal{T}_{\mathbb{A}}^k(z_0))\right)$.
By strict telescoping, since $\mathcal{T}_{\mathbb{A}}^L(z_0) = z_0$, the exact sum vanishes, yielding $\oint_\gamma \omega = 0$.

**Step 3: Cohomological triviality via the adelic Lefschetz trace formula**
Consider the action of $\mathcal{T}_{\mathbb{A}}$ on the sheaf of locally constant functions.
By the trace formula, the weighted sum of periodic cycles is equal to the induced trace in cohomology.
Since the cohomological boundary $\partial$ is trivial on $\gamma$, the cohomology class $[\gamma] \in \mathcal{H}^1(\mathcal{G}_{\mathbb{A}}, \mathbb{Z})$ is null.
Thus, any cycle retracts cohomologically to the base point of the fibration.

**Step 4: Topological uniqueness of the trivial cycle (4, 2, 1)**
The orbit $\{1, 2, 4\}$ constitutes the canonical generator of the main connected component of the bundle.
Any other hypothetical loop would reside in a null cohomological class. However, the fibration energy (Lemma 10) induces a strictly positive metric on the loop space, making it impossible for a non-trivial loop of arbitrary length to exist without injecting infinite adelic energy.
...
### Lemma 50 : Absolute continuity of the Perron-Frobenius eigenmeasure associated with $\mathcal{L}$

**Statement :**
Let $\lambda_0 \in \mathbb{R}_{>0}$ be the spectral radius of the adelic transfer operator $\mathcal{L}$ on $L^2(\mathcal{G}_{\mathbb{A}})$, and let $\nu_{\mathbb{A}}$ be the unique eigen-probability measure satisfying $\mathcal{L}^* \nu_{\mathbb{A}} = \lambda_0 \nu_{\mathbb{A}}$ (Perron-Frobenius measure). Then the measure $\nu_{\mathbb{A}}$ is absolutely continuous with respect to the invariant Haar measure $\mu_{\mathbb{A}}$ on $\mathcal{G}_{\mathbb{A}}$.

**Proof :**

**Step 1 : Construction of the invariant density**
By the Ruelle-Perron-Frobenius theorem adapted to the adelic formalism, there exists a unique strictly positive eigenfunction $h \in L^2(\mathcal{G}_{\mathbb{A}}, \mathbb{R}_{>0})$ associated with the maximal eigenvalue $\lambda_0$, such that $\mathcal{L}(h) = \lambda_0 h$. The dual eigenmeasure is $\nu_{\mathbb{A}}$.

**Step 2 : Regularity of the eigenfunction**
The transfer operator $\mathcal{L}$ acts on the space of Hölder continuous functions on $\mathcal{G}_{\mathbb{A}}$. By the spectral gap theorem for adelic hyperbolic dynamics, the eigenspace associated with the eigenvalue $\lambda_0$ is one-dimensional and is generated by a function belonging to this regularity space. Consequently, $h$ is a globally continuous function on the compact topological group $\mathcal{G}_{\mathbb{A}}$.

**Step 3 : Equivalence relation of the measures**
We define a new measure $m_{\mathbb{A}}$ by $dm_{\mathbb{A}}(x) = h(x) d\mu_{\mathbb{A}}(x)$. Since $h$ is strictly positive and continuous, and $\mathcal{G}_{\mathbb{A}}$ is compact, $h$ is bounded below and above by strictly positive constants : $0 < C_1 \leq h(x) \leq C_2 < \infty$. The measure $m_{\mathbb{A}}$ is thus equivalent to the Haar measure $\mu_{\mathbb{A}}$.

**Step 4 : Invariance of the equilibrium measure**
...
**Step 1: Ergodic decomposition of the invariant measure**
Let $\eta_{\mathbb{A}}$ be a probability measure invariant under the operator $\mathcal{T}_{\mathbb{A}}$, such that $\eta_{\mathbb{A}} \ll m_{\mathbb{A}}$.
By the ergodic decomposition theorem, there exists a family of ergodic invariant probability measures $\{\eta_{\omega}\}_{\omega \in \Omega}$ and a spectral probability measure $\mathbb{P}$ on $\Omega$ such that for any measurable set $B \subset \mathcal{G}_{\mathbb{A}}$, we have:
$\eta_{\mathbb{A}}(B) = \int_{\Omega} \eta_{\omega}(B) d\mathbb{P}(\omega)$.

**Step 2: Density and measure equivalence**
Since $\eta_{\mathbb{A}} \ll m_{\mathbb{A}}$, it follows that for $\mathbb{P}$-almost every $\omega \in \Omega$, the ergodic component $\eta_{\omega}$ is also absolutely continuous with respect to $m_{\mathbb{A}}$, i.e., $\eta_{\omega} \ll m_{\mathbb{A}}$.
Recall from Lemma 50 that the Perron-Frobenius measure $\nu_{\mathbb{A}}$ is itself absolutely continuous with respect to $m_{\mathbb{A}}$, and corresponds to the Gibbs measure for the Hölder potential of the operator $\mathcal{T}_{\mathbb{A}}$.

**Step 3: Uniqueness of the absolutely continuous ergodic measure**
For a contracting hyperbolic dynamical system as defined on the adelic fibration (Lemma 19), there exists at most one ergodic invariant probability measure that is absolutely continuous with respect to the adelic Lebesgue measure.
Let $\eta_{\omega}$ be such an ergodic component. Its density function $\rho_{\omega} = \frac{d\eta_{\omega}}{dm_{\mathbb{A}}}$ must satisfy the local Perron-Frobenius functional equation:
$\mathcal{L}(\rho_{\omega}) = \rho_{\omega}$,
where $\mathcal{L}$ is the adelic transfer operator.
However, Lemma 50 established that the eigenspace associated with the maximal eigenvalue $\lambda_0 = 1$ (after normalization) is of strict dimension 1, generated by the density of $\nu_{\mathbb{A}}$.
Consequently, $\rho_{\omega}$ must be proportional to the density of $\nu_{\mathbb{A}}$. Both being probability measures, the constant of proportionality is 1, so $\eta_{\omega} = \nu_{\mathbb{A}}$.

**Step 4: Conclusion by integration**
Since for almost every $\omega \in \Omega$, $\eta_{\omega} = \nu_{\mathbb{A}}$, the ergodic decomposition integral simplifies to:
...
### Lemma 54: Completeness of the Adelic Wavelet Basis on $L^2(\mathbb{A}_{\mathbb{Q}}^{\mathcal{S}})$

**Definition 54.1:**
Let the Hilbert space be $L^2(\mathbb{A}_{\mathbb{Q}}^{\mathcal{S}}, \mu_{\mathbb{A}})$. We define the adelic wavelet system $\mathcal{W}_{\mathbb{A}} = \{ \psi_{j, k, p} \}_{j,k,p}$ generated by the mother wavelet $\psi \in L^2(\mathbb{A}_{\mathbb{Q}}^{\mathcal{S}})$, as defined during the spectral construction (cf. Lemma 40). The family is generated by the scaling operators $\mathcal{S}_p$ and translation operators $\tau_k$.

**Statement of Lemma 54:**
The adelic wavelet system $\mathcal{W}_{\mathbb{A}}$ forms a complete orthonormal basis of the Hilbert space $L^2(\mathbb{A}_{\mathbb{Q}}^{\mathcal{S}}, \mu_{\mathbb{A}})$. Consequently, the orthogonal projection $\Pi_{\mathcal{W}} : L^2(\mathbb{A}_{\mathbb{Q}}^{\mathcal{S}}) \to \overline{\text{Span}(\mathcal{W}_{\mathbb{A}})}$ is exactly the identity operator $\text{Id}_{L^2}$.

**Proof of Lemma 54:**

**Step 1: Orthonormality of the family $\mathcal{W}_{\mathbb{A}}$**
Let $\psi_{j_1, k_1, p_1}$ and $\psi_{j_2, k_2, p_2}$ be two distinct elements of $\mathcal{W}_{\mathbb{A}}$. By construction of the adelic multiresolution analysis on locally compact groups (Theorem of Adelic Harmonic Analysis), the scaling spaces $V_j$ satisfy $V_j \subset V_{j+1}$ and the wavelet spaces $W_j$ are defined as the orthogonal complement of $V_j$ in $V_{j+1}$.
Thus, if $j_1 \neq j_2$, say $j_1 < j_2$, we have $\psi_{j_1, k_1, p_1} \in W_{j_1} \subset V_{j_1+1} \subset V_{j_2}$.
Since $W_{j_2} \perp V_{j_2}$, it follows that $\langle \psi_{j_1, k_1, p_1}, \psi_{j_2, k_2, p_2} \rangle_{L^2} = 0$.
If $j_1 = j_2$, the orthonormality within the same scale is guaranteed by the translation invariance of the adelic lattice.
Thus, the family is rigorously orthonormal.

**Step 2: Density of the spanned subspace**
Assume, for the sake of contradiction, that $\mathcal{W}_{\mathbb{A}}$ is not complete. There then exists a non-zero function $f \in L^2(\mathbb{A}_{\mathbb{Q}}^{\mathcal{S}})$ such that $f \perp \overline{\text{Span}(\mathcal{W}_{\mathbb{A}})}$.
Consequently, for all $j, k, p$, we have $\langle f, \psi_{j, k, p} \rangle_{L^2} = 0$.
By the properties of multiresolution analysis, this implies that the projection of $f$ onto any scaling space $V_j$ is zero, that is to say $\Pi_{V_j}f = 0$ for all $j \in \mathbb{Z}$.

**Step 3: Convergence of the projections**
According to the axiom of the dense union of scaling spaces on the adeles, we have $\overline{\bigcup_{j \in \mathbb{Z}} V_j} = L^2(\mathbb{A}_{\mathbb{Q}}^{\mathcal{S}})$.
It follows that $\lim_{j \to +\infty} \Pi_{V_j}f = f$ in the $L^2$ topology.
Since $\Pi_{V_j}f = 0$ for all $j$, we obtain that $f = 0$ almost everywhere with respect to the measure $\mu_{\mathbb{A}}$.
This contradicts the hypothesis that $f$ is non-zero.
...
### Proof of Lemma 63 (Invariance under Galois Automorphism)

**Step 1: Axiomatization of the Galois Action**
Let $G_K = \text{Gal}(\overline{\mathbb{Q}}/\mathbb{Q})$ be the absolute Galois group. We define the action of $G_K$ on the adelic fibration $\mathbb{A}_{\mathbb{Q}}^{\mathcal{S}}$ by the natural extension of automorphisms on local algebraic completions. For any element $\sigma \in G_K$ and any regular point $x \in \mathbb{A}_{\mathbb{Q}}^{\mathcal{S}}$, the action is given by the composition $\sigma \cdot x = (\sigma_p(x_p))_{p \in \mathcal{P} \cup \{\infty\}}$.
We state that the adelic Collatz operator $\mathcal{T}_{\mathbb{A}}$ commutes with the action of $G_K$, i.e., $\mathcal{T}_{\mathbb{A}}(\sigma \cdot x) = \sigma \cdot \mathcal{T}_{\mathbb{A}}(x)$.

**Step 2: Evaluation of Galois-Conjugate Orbits**
Suppose, for the sake of contradiction, that there exists an asymmetrical rational orbit with respect to the action of $G_K$. If $x \in \mathbb{Q}$ generates a periodic trajectory, its algebraic closure under the action of $\mathcal{T}_{\mathbb{A}}$ should form a finite Galois extension (Lemma 48).
However, the strict 2-adic isometry (Lemma 55) dictates that any non-trivial conjugation would perturb the cohomological equivalence demonstrated in Lemma 61. The evaluation of the cocycle $c_{\sigma \cdot x, k}$ for a conjugate orbit would lead to a distinct class $[c_{\sigma \cdot x, k}]$ in $H^1(\mathcal{A}_{triv}, \mathbb{Z}_2)$, contradicting the uniqueness of the trivial class associated with $\mathcal{A}_{triv}$.

**Step 3: Restriction to Pure Rationals**
Since any cyclic orbit generated by a rational $x > 4$ must induce a globally null cohomology class, and any action of $G_K$ preserves this annihilation, the orbit must be invariant under any Galois conjugation. Formally, $\sigma \cdot \mathcal{T}_{\mathbb{A}}^n(x) = \mathcal{T}_{\mathbb{A}}^n(x)$ for all $\sigma \in G_K$.
This implies that all elements of the orbit are strictly unramified rational numbers. The absence of Galois ramification for non-trivial orbits confirms that such orbits cannot emerge from an underlying algebraic structure beyond the fundamental cycle $\{1, 2, 4\}$.

**Step 4: Conclusion**
The commutation of $\mathcal{T}_{\mathbb{A}}$ with the action of $G_K$ and the cohomological invariance require that no rational orbit distinct from $\mathcal{A}_{triv}$ can sustain an asymmetrical cyclic structure. The proof of Lemma 63 is rigorously completed.

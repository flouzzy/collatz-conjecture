...
**Lemma 37 (Isomorphism of Integer Trajectories and Resolution of the Collatz Conjecture):**
Let the canonical inclusion of strictly positive natural integers be $\iota : \mathbb{N}^* \hookrightarrow \mathbb{A}_{\mathbb{Q}}^{\mathcal{S}}$. For any integer $N \in \mathbb{N}^*$, the orbit generated by the standard Collatz operator on $\mathbb{N}^*$ is strictly isomorphic in time and trajectory to the projected orbit of its image $\iota(N)$ under the adelic operator $\mathcal{T}_{\mathbb{A}}$. Since the exceptional set $\mathcal{E}$ is strictly empty (Lemma 36), the image of any integer initial condition is absorbed by the trivial attractor $\mathcal{A}_{triv}$ in a finite number of steps. By metric isomorphism, the corresponding integer trajectory inevitably reaches the trivial cycle $\{1, 4, 2\}$, definitively proving the universal validity of the Collatz Conjecture.
...
### Proof of Lemma 37 (Isomorphism of Integer Trajectories and Resolution of the Collatz Conjecture)

**Step 1: Construction of the canonical immersion**
Let $\mathbb{N}^*$ be the set of strictly positive natural integers. We define the diagonal injection $\iota : \mathbb{N}^* \hookrightarrow \mathbb{A}_{\mathbb{Q}}^{\mathcal{S}}$ such that for any $N \in \mathbb{N}^*$, $\iota(N)$ has the value $N$ on all places (archimedean and $p$-adic). By definition of the restricted fractional adelic space $\mathbb{A}_{\mathbb{Q}}^{\mathcal{S}}$, $\iota(N)$ is a well-defined element.

**Step 2: Commutativity of the operators**
Let $C : \mathbb{N}^* \to \mathbb{N}^*$ be the standard Collatz operator. By construction in Axiom 1 (Adelic Transition Operator), for any $x \in \iota(\mathbb{N}^*)$, the parity condition (determined by the 2-adic valuation of the component in $\mathbb{Q}_2$) coincides exactly with the parity of the integer $N$.
Thus, the local action of $\mathcal{T}_{\mathbb{A}}$ on each place is arithmetically identical to the action of $C$. Therefore, we have the strict commutative diagram: $\mathcal{T}_{\mathbb{A}} \circ \iota = \iota \circ C$.
By induction, for any $k \in \mathbb{N}$, $\mathcal{T}_{\mathbb{A}}^k(\iota(N)) = \iota(C^k(N))$.

**Step 3: Finiteness and attraction**
According to Lemma 36, the exceptional set $\mathcal{E}$, consisting of points whose adelic orbit does not converge to the trivial cycle $\mathcal{A}_{triv}$, is strictly empty ($\mathcal{E} = \emptyset$).
Since $\iota(N) \in \mathbb{A}_{\mathbb{Q}}^{\mathcal{S}}$, it follows that $\iota(N) \notin \mathcal{E}$.
Consequently, the adelic trajectory $(\mathcal{T}_{\mathbb{A}}^k(\iota(N)))_{k \in \mathbb{N}}$ eventually joins the trivial cycle $\mathcal{A}_{triv}$ in a finite flight time $K = \tau_{\mathbb{A}}(\iota(N)) < \infty$.

**Step 4: Isomorphism of the attractors**
The image of the trivial integer cycle $\{1, 4, 2\}$ by the injection $\iota$ is exactly the attractor $\mathcal{A}_{triv}$ in the adelic space.
Since $\mathcal{T}_{\mathbb{A}}^K(\iota(N)) \in \mathcal{A}_{triv}$, by the strict injectivity of $\iota$ on the domain of integers, it follows that $C^K(N) \in \{1, 4, 2\}$.
The integer orbit therefore reaches the cycle $\{1, 4, 2\}$ in exactly $K$ iterations.

**Step 5: Conclusion**
Since the choice of the initial condition $N \in \mathbb{N}^*$ is entirely arbitrary, it is rigorously proven that any strictly positive natural integer reaches the trivial cycle $\{1, 4, 2\}$ in a finite number of steps.
The Collatz Conjecture is thus universally demonstrated as a fundamental theorem resulting from the topological and dynamical rigidity of the adelic fibration.
